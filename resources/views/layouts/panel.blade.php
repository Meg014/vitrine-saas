...
<div class="min-h-screen lg:flex">
    ...
    <aside :class="menu ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 p-5 text-white transition-transform lg:static lg:translate-x-0">
        ...
        @php($links = [['Visão geral','dashboard','⌂'],['Produtos','products.index','◇'],['Categorias','categories.index','▦'],['Atributos','attributes.index','✦'],['Estoque','inventory.index','▤'],['Pedidos','orders.index','▤'],['Clientes','customers.index','♙'],['Mensagens','messages.index','✉'],['Aparência','aparencia','✦'],['Configurações','configuracoes','⚙']])
        <nav class="mt-6 flex-1 space-y-1 overflow-y-auto">@foreach($links as [$label,$route,$icon])@php($named=str_contains($route,'.')||$route==='dashboard')@php($active=$named ? request()->routeIs(str_replace('.index','.*',$route)) : request()->route('page') === $route)<a class="nav-link {{ $active ? 'nav-link-active' : '' }}" href="{{ $named ? route($route) : route('panel.page',$route) }}"><span class="w-5 text-center">{{ $icon }}</span>{{ $label }}</a>@endforeach</nav>
